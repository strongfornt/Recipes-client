      ...
      if (singleUser?.coin > 10) {
        ...
        axiosPublic.put("/recipe", recipeInfo).then((res) => {
          ...
          if (res.data.modifiedCount) {
            //user coin update related api call if user currently not exist on the purchased array ======
            axiosPublic.put("/users", userInfo).then((res) => {
                if (
                  res.data.resultBuyer.modifiedCount &&
                  res.data.resultCreator.modifiedCount
                ) {
                  refetch();
                  setUserRefetch(!userRefetch);
                }
              });
              //user coin update related api call if user currently not exist on the purchased array end ======
            ...
            <div className="w-full lg:mr-2 lg:w-1/2">
              <div className="lg:max-w-lg">
                ...
                <button
                  ...
                  className="w-full px-5 py-2 mt-6 text-sm tracking-wider font-bold text-white uppercase transition-colors duration-300 transform bg-teal-600 rounded-lg lg:w-auto hover:bg-teal-500 focus:outline-none focus:bg-teal-500"
                >
                  View The Recipe
                </button>
              </div>
